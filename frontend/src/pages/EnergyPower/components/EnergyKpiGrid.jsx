  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 font-sans">
      {kpis.map((kpi, idx) => {
        const Icon = kpi.icon;
        return (
          <div key={idx} className="flex flex-col sm:flex-row items-start sm:items-center gap-2.5 sm:gap-4 rounded-2xl border border-slate-200/80 bg-white/70 backdrop-blur-md p-3.5 sm:p-5 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] dark:border-slate-800/80 dark:bg-slate-950/60 transition-colors duration-300 overflow-hidden">
            ...
            <div className="min-w-0 w-full">
              ...
              <div className="flex items-baseline gap-2 mt-0.5 sm:mt-1 truncate">
                <p className="font-mono text-sm sm:text-xl font-bold text-slate-900 dark:text-slate-100 truncate">{kpi.value}</p>
                {kpi.subtext && (
                  <span className={`hidden xl:inline-block font-mono text-[0.6rem] uppercase tracking-wider ${statusColors[kpi.status]}`}>
                    {kpi.subtext}
                  </span>
                )}
              </div>
